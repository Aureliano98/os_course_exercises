...
3. 禁用中断是如何实现对临界区的访问控制的？有什么优缺点？
   ...
   * 无法中断，临界区代码故障可以导致系统崩溃
   * 其他进程可能出现饥饿；
   * 只适用于单处理器

4. Test&Set原子操作是否可以实现Exchange原子操作? Exchange原子操作是否可以实现Test&Set原子操作? 这两条指令在实现原子操作的功能是等价的（见17.5第1问）。
...
有误。能满足忙则等待，不满足空闲则入（两个进程必须交替进入临界区）。
...
有误。两个进程可能同时进入entry protocol，然后在while循环死锁。
...
有误。与7没有本质不同，定期释放检查虽然减少了死锁的可能，但不能保证不死锁。
...
正确。并非证明，但与7相比，直观来看即使两个进程都进入外层while循环也只有一个进程会在内层while等待，这使得另一个进程跳出外层while循环，并且将turn交给等待进程。
...
似乎是正确的（与piazza讨论不一致）。对于每个count值，都会使得最后一个修改turn的进程进入等待，直到(1) 所有在更小的count值就陷入等待的进程 (2) 在该count值不等待的其他进程 都完成后，该进程才能继续执行count+1。
...
正确。见<https://piazza.com/class/i5j09fnsl7k5x0?cid=1060>
...
1. 如何证明TS指令和交换指令的等价性？
   ...
   ```Java
   void swapByTestAndSet(Boolean a, Boolean b) {
   ...
   ```Java
   Boolean testAndSetBySwap(Boolean flag) {
   	Boolean tmp = true;
   	swap(flag, tmp);
       return tmp;
   }
